        ...
        files: [
          {
            expand: true,
            cwd: '<%= config.sassDir %>',
            src: ['**/*.scss', '!<%= config.sassMainFileName %>.scss','!bootstrap.scss'],
            dest: '<%= config.cssDir %>',
            ext: '.css'
          },
          {src: '<%= config.sassDir %><%= config.sassMainFileName %>.scss', dest: '<%= config.cssMainFileDir %><%= config.cssMainFileName %>.css'},
          {src: '<%= config.sassDir %>bootstrap.scss', dest: '<%= config.cssDir %>bootstrap.min.css'}
        ]
      },
      dist: {
        options: {
...
//finally
  //css beautiful
  grunt.registerTask('cssbeauty', ['sass:dist', 'combine_mq:dist', 'postcss:dist', 'csscomb:dist']);
  //img minify
  grunt.registerTask('imgmin', ['imagemin', 'pngmin:all', 'svgmin']);

  //final build
  grunt.registerTask('dist', ['clean:temp', 'sass:min', 'imgmin', 'cssbeauty']);
